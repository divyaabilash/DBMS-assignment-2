SELECT X.BUSINESS_CAT_ID,X.TOP_TWO_BUSINESS_ID,X.TOP_TWO_BUSINESS_NAME,Y.NAME FROM (SELECT BUSINESS_CAT_ID,LISTAGG(BUSINESS_ID,',') WITHIN GROUP(ORDER BY BUSINESS_CAT_ID) AS TOP_TWO_BUSINESS_ID,LISTAGG(BUSINESS_NAME,',') WITHIN GROUP(ORDER BY BUSINESS_CAT_ID) AS TOP_TWO_BUSINESS_NAME FROM (SELECT DISTINCT BUSINESS_CAT_ID,BUSINESS_ID,BUSINESS_NAME FROM 
