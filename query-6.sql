SELECT X.BUSINESS_CAT_ID,Y.NAME AS CATEGORY_NAME,X.TOP_BUSINESS_IDS,X.TOP_BUSINESS_NAMES FROM (
SELECT BUSINESS_CAT_ID,LISTAGG(BUSINESS_ID,',') WITHIN GROUP(ORDER BY BUSINESS_CAT_ID) AS TOP_BUSINESS_IDS,LISTAGG(BUSINESS_NAME,',') WITHIN GROUP(ORDER BY BUSINESS_CAT_ID) AS TOP_BUSINESS_NAMES FROM (
